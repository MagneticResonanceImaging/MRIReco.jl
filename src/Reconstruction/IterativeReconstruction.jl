...
function reconstruction_lowRank(aqData::AquisitionData, recoParams::Dict)
  # signal encoding transformation
  recoParams[:fft] = true
  E = lrEncodingOp(aqData, recoParams, numEchoes=aqData.numEchoes, parallel=true)
  ...
  C = aqData.numCoils
  S = aqData.numSlices
  subspace = get( recoParams, :phi, Matrix{Float64}(I,prod(recoParams[:shape]),prod(recoParams[:shape])) )
  K = size(subspace,2)
  Φ = MapSliceOp(subspace[:,:,1],2,(N, K), (N, T))

  # sparsifying transform
  recoParams[:multiEcho] = true
  recoParams[:numEchoes] = T
  sparseTrafoName = get(recoParams, :sparseTrafoName, "nothing")
  if sparseTrafoName != "nothing"
    recoParams[:sparseTrafo] = SparseOp(sparseTrafoName; recoParams...)
  end
  ...
  Ireco = zeros(ComplexF64, N, T, S)
  solvername = get(recoParams,:solver,"fista")
  solver = createLinearSolver(solvername, E, reg; recoParams...)
  for i = 1:S
    kdata = multiCoilMultiEchoData(aqData,i)
    α = solve(solver, kdata)
    Ireco[:,:,i] = reshape(Φ*α, N, T)
  end

  Ireco = reshape( permutedims(Ireco, [1,3,2]), recoParams[:shape]..., S, T )
end
